...
const MakeReservationIntentHandler = {
	...
	handle(handlerInput) {
		...
		if (!restaurantName.value || !date.value || !time.value || !numPeople.value)
			return handlerInput.responseBuilder.speak("Missing parameters,").reprompt("Again missing parameters,").addDelegateDirective(currentIntent).getResponse()
